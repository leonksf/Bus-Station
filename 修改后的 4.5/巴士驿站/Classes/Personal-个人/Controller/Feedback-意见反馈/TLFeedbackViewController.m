...
#import "Masonry.h"
@interface TLFeedbackViewController ()<UITextViewDelegate,UICollectionViewDelegate,UICollectionViewDataSource, UICollectionViewDelegateFlowLayout,UIImagePickerControllerDelegate,UINavigationControllerDelegate>
...
- (void)viewDidLoad {
    ...
    self.wordCountLabel.backgroundColor=[UIColor blackColor];
    ...
    self.photoBtn.frame = CGRectMake(10, 190, (self.view.frame.size.width- 60) / 5, (self.view.frame.size.width- 60) / 5);
    [_photoBtn setImage:[UIImage imageNamed:@"意见反馈"] forState:UIControlStateNormal];
    [_photoBtn setBackgroundColor: [UIColor lightGrayColor]];
...
//        _aView.frame = CGRectMake(20, 84, self.view.frame.size.width - 40, 180);
        self.photoBtn.frame = CGRectMake(10 * (self.photoArrayM.count + 1) + (self.view.frame.size.width - 60) / 5 * self.photoArrayM.count, 190, (self.view.frame.size.width - 60) / 5, (self.view.frame.size.width - 60) / 5 );
...
-(void)addCollectionViewPicture{
    ...
    //创建集合视图
    self.collectionV = [[UICollectionView alloc]initWithFrame:CGRectMake(0, 100, self.view.frame.size.width, ([UIScreen mainScreen].bounds.size.width - 60) / 5 + 10) collectionViewLayout:flowL];
...
-(void)addContactInformation{
    ...
    [_textField mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.collectionV.mas_bottom).offset(50);
        make.width.equalTo(self.view.mas_width);
...
-(PlaceholderTextView *)textView{
    ...
    if (!_textView) {
        _textView = [[PlaceholderTextView alloc]initWithFrame:CGRectMake(0, 65, self.view.frame.size.width, 100)];
...
- (UIButton *)sendButton{
    ...
    if (!_sendButton) {
        ...
        [_sendButton setBackgroundColor:[UIColor colorWithRed:39/255.0 green:124/255.0 blue:255/255.0 alpha:0.75]];
