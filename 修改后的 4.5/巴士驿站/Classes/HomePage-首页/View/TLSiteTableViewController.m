...
@interface TLSiteTableViewController ()<UISearchResultsUpdating>
...
@implementation TLSiteTableViewController

-(NSArray *)data{
    
    if (_siteList == nil) {
        NSString *file = [[NSBundle mainBundle]pathForResource:@"Address.plist" ofType:nil];
        NSArray *dictArray = [NSArray arrayWithContentsOfFile:file];
        NSMutableArray *arraryM = [NSMutableArray array];
        for (NSDictionary *dict in dictArray) {
            TLProvinces *data = [[TLProvinces alloc]initWithDict:dict];
            [arraryM addObject:data];
...
- (void)updateSearchResultsForSearchController:(UISearchController *)searchController {
    ...
    self.searchList = [[self.siteList filteredArrayUsingPredicate:searchPredicate] mutableCopy];
